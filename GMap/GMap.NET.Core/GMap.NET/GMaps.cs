   ...
   public class GMaps : Singleton<GMaps>
   {
      ...
      internal string MakeImageUrl(MapType type, Point pos, int zoom, string language)
      {
         switch(type)
         {
            ...
            case MapType.UMP:
            {
                // http://1.tiles.ump.waw.pl/ump_tiles/7/71/40.png
                char letter = "123"[GetServerNum(pos, 3)];
                return string.Format("http://{0}.tiles.ump.waw.pl/ump_tiles/{1}/{2}/{3}.png", letter, zoom, pos.X, pos.Y);
            }
